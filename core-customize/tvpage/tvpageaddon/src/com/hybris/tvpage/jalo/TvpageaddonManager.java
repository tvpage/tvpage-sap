...
/**
 * This is the extension manager of the Tvpageaddon extension.
 */
public class TvpageaddonManager extends GeneratedTvpageaddonManager {
    /**
     * Edit the local|project.properties to change logging behavior (properties 'log4j.*').
     */
    private static final Logger LOG = Logger.getLogger(TvpageaddonManager.class.getName());
     ...
     */
    public static TvpageaddonManager getInstance() {
        return (TvpageaddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
                TvpageaddonConstants.EXTENSIONNAME);
    }
     ...
     */
    public TvpageaddonManager() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("constructor of TvpageaddonManager called.");
        }
    }
    ...
    public void init() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("init() of TvpageaddonManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("destroy() of TvpageaddonManager called, current tenant: " + getTenant().getTenantID());
        }
    }
